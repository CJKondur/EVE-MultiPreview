...
Class LogMonitor {
    ...
    static SCAN_INTERVAL_MS := 300000  ; 5 minutes — rescan for new log files
    ...
    Start() {
        ...
        ; Delayed re-scan: HandleMainTimer runs every 50ms and populates
        ...
        try {
            ...
            loop read state.filePath {
                if (InStr(A_LoopReadLine, "Channel changed to Local")) {
                    ; Extract system name from "Channel changed to Local : SystemName"
    ...
    _ReadNewLines(state) {
        try {
            currentSize := FileGetSize(state.filePath)
            currentMod := FileGetTime(state.filePath, "M")
        } catch
            return false

        ; No change
        if (currentSize = state.size && currentMod = state.lastMod)
            return false
        ...
        ; Nothing new to read
        if (currentSize <= state.pos) {
            state.size := currentSize
            state.lastMod := currentMod
            return false
        }
        ...
        try {
            f := FileOpen(state.filePath, "r")
            f.Seek(state.pos)
            newData := f.Read(currentSize - state.pos)
            f.Close()
        } catch
            return false

        state.pos := currentSize
        state.size := currentSize
        state.lastMod := currentMod
    ...
    _EmitEvent(charName, eventType, severity, text) {
        ...
        ; Tray notification for CRITICAL and WARNING
        if (severity = LogMonitor.SEV_CRITICAL) {
            try TrayTip(text, "EVE Alert — " charName, "16")
        } else if (severity = LogMonitor.SEV_WARNING) {
            try {
                if (this._GetSeverityTrayNotify(severity))
                    TrayTip(text, "EVE Alert — " charName, "17")
            }
        }
